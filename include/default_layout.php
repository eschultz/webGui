...
$(document).ready(function() {
  ...
  <?if ($confirm['warn']):?>
    $('form').each(function(){$(this).change(function(){$.jGrowl('You have uncommitted form changes',{sticky:false, theme:'bottom', position:'bottom', life:2000});});});
  <?endif;?>
});
...
<div id="template">
    ...
    <div id="header" class="<?=$display['banner'];?>">
      <div class="logo">
        <a href="http://lime-technology.com"><img src="/plugins/webGui/images/logo.png" title="unRAID" border="0"/><br/>
           <strong>unRAID Server <em><?=$var['regTy'];?></em></strong>
        </a>
      </div>
      <div class="block">
        ...
        <div id="nav_left">
          <?php
            ...
            foreach ($pages as $page) {
              ...
              $link = "/{$page['name']}";
              if ($page['name']==$task)
                echo "<div id=\"nav_item\" class=\"active\"><a href=\"$link\">{$page['name']}</a></div>";
              else
                echo "<div id=\"nav_item\"><a href=\"$link\">{$page['name']}</a></div>";
            }
          ?>
        </div>
        <div id="nav_right">
          <?php
            ...
            foreach ($pages as $page) {
              $src = icon_file($page);
              eval("?>{$page['text']}");
              echo "<div id=\"nav_item\" class=\"{$page['name']}_button\">";
              echo "<a href=\"#\" onclick=\"{$page['name']}(); return false;\"><img src=\"/$src\">&nbsp;{$page['Title']}</a>";
              echo "</div>";
            }
          ?>
        </div>
  ...
  <div id="footer">
    <div id="copyright">unRAID&#8482; webGui &copy; Lime Technology";
